...
function Traverse {
    ...
    $Moves | % {
        $op = $_[0]
        $length = [int]::Parse($_.SubString(1))

        Write-Verbose "New OP: $op + $length"

        switch ($op)
        {
            'U' {
                TraverseSteps -Vertical -X $currentX -Y $currentY -Map $map -Length $length -WireName $WireName -EmitIntersection:$EmitIntersection
                $currentY += $length
            }
            'D' {
                TraverseSteps -Vertical -X $currentX -Y $currentY -Map $map -Length ($length * -1) -WireName $WireName -EmitIntersection:$EmitIntersection
                $currentY -= $length
            }
            'L' {
                TraverseSteps -Horizontal -X $currentX -Y $currentY -Map $map -Length ($length * -1) -WireName $WireName -EmitIntersection:$EmitIntersection
                $currentX -= $length
            }
            'R' {
                TraverseSteps -Horizontal -X $currentX -Y $currentY -Map $map -Length $length -WireName $WireName -EmitIntersection:$EmitIntersection
                $currentX += $length
            }
        }
    }

    if (-not($EmitIntersection))
    {
        $map
    }
}
...
function TraverseSteps
{
    ...
    $increment = if ($Length -gt 0) { 1 } else { -1 }
    $Length = [Math]::Abs($Length)

    $cumulativeLength = if ($null -eq $Map[$X]) { 1 } else { $Map[$X][$Y].$WireName + 1 }
    $currentX = $X
    $currentY = $Y
    ...
    for ($i = 0; $i -lt $Length; $i++)
    {
        ...
        if ($null -eq $Map[$currentX])
        {
            $Map[$currentX] = @{}
        }

        if ($null -eq $Map[$currentX][$currentY])
        {
            ...
            $visit.$WireName = $cumulativeLength++;
        ...
        else
        {
            ...
            $Map[$currentX][$currentY].$WireName = $cumulativeLength++

            if ($EmitIntersection -and $Map[$currentX][$currentY].Wire1 -gt 0 -and $Map[$currentX][$currentY].Wire2 -gt 0)
            {
                Write-Verbose "Found intersection at ($currentX, $currentY), Wire1 length: $($Map[$currentX][$currentY].Wire1), Wire2 length: $($Map[$currentX][$currentY].Wire2)"
                $Map[$currentX][$currentY]
            }
        }
    }

    Write-Verbose "$debug, End ($currentX, $currentY)"
}

$map = @{}

Write-Host "Part 1"
$map = Traverse -Map $map -Moves $wire1 -WireName "Wire1"

$intersections = Traverse -Map $map -Moves $wire2 -WireName "Wire2" -EmitIntersection:$true
